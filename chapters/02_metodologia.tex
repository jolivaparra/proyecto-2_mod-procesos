...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/diagramas/diagrama-lazo-abierto.pdf}
...
El diagrama esquemático del proceso se modifica integrando el bloque del controlador lógico, tal como se describe a continuación.
...
\subsection{Temporales (Respuesta Transitoria)}
...
\begin{equation}
    T_s = \min \{ t \mid |T_p(t) - T_{final}| \leq 0.01 \cdot \Delta T_{total} \}
\end{equation}
...
Para evaluar el desempeño energético del sistema con coontrolador implementado, es necesario cuantificar el consumo total de energía eléctrica por parte de los actuadores durante un ciclo operativo completo (un día simulado).

Si bien en el proyecto pasado se tomó un modelo específico de bomba de agua, y se tomó valores arbitrarios para el ventilador, en este proyecto se tomará una aproximación para la potencia consumida por cada actuador en función de la tensión aplicada a estos, asumiendo que la corriente consumida es proporcional a la tensión aplicada (modelo lineal simplificado).

\section{Escenarios de Simulación} \label{sec:escenarions-de-simulacion}
...
\subsection{Día Parcialmente Nublado (Intermitente)}
...
\subsection{Entorno Controlado para Análisis de Métricas} \label{subsecc:entorno-controlado}

Para aplicar correctamente las métricas de desempeño temporal definidas en la sección anterior, se efectuan los cálculos y mediciones sobre un entorno de simulación controlada. Este entorno se caracteriza por mantener condiciones ambientales constantes durante el periodo de análisis, eliminando las variaciones diurnas y perturbaciones externas.
...
\section{Pruebas/Simulaciones a Realizar}
...
\begin{itemize}
    ...
    \item \textbf{Simulación en Entorno Controlado:} Se llevará a cabo una simulación específica en el entorno controlado descrito en la sección \ref{subsecc:entorno-controlado}. Esta prueba se centrará en la evaluación de las métricas temporales de desempeño del sistema, permitiendo un análisis detallado.
