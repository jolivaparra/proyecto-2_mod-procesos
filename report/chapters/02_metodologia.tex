...
\begin{equation}
    E_{total} = \int_{0}^{T} \left( \frac{V_{bomb}(t)^2}{R_{bomb}} + \frac{V_{vent}(t)^2}{R_{vent}} \right) dt \quad [\si{\joule}]
\end{equation}
...
\begin{itemize}
    \item $\mathbf{R_\text{vent}}$: Los ventiladores trabajan comunmente potencias entre \SI{5}{\watt} y \SI{15}{\watt}, para el ventilador se tomó una aproximación de \SI{12}{\watt} de potencia nomilan, de esta forma podemos realizar aproximaciones de la resistencia equivalente:
          \begin{equation*}
              P_\text{max, venr} = \frac{(V_\text{nom})^2}{R_\text{vent}} \implies \SI{12}{\watt} = \frac{(\SI{12}{\volt})^2}{R_\text{vent}} \implies \boxed{R_\text{vent} = \SI{12}{\ohm}}
          \end{equation*}
...
\subsection{Escenarios de Simulación} \label{subsec:escenarions-de-simulacion}
...
\subsubsection{Entorno Controlado para Análisis de Métricas} \label{subsecc:entorno-controlado}
